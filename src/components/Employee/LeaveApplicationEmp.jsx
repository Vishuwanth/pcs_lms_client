...
import { isToday, compareDesc, differenceInDays } from "date-fns";
...
function LeaveApplicationEmp(props) {
      ...
      if (fromDateFNS.getUTCFullYear() >= todaysDate.getUTCFullYear()) {
        if (fromDateFNS.getMonth() >= todaysDate.getMonth()) {
          if (fromDateFNS.getUTCDate() >= todaysDate.getUTCDate()) {
            ...
            if (localStorage.getItem("leaveBalance") >= noOfDays) {
              axios
                .post(
                  `${apiUrl}/leave-application-emp/` + props.data["_id"],
                  body,
                  {
                    headers: {
                      authorization: localStorage.getItem("token") || "",
              ...
              window.alert(
                "You dont have sufficient Leave Balance,Leave Balance is" +
                  " " +
                  localStorage.getItem("leaveBalance")
              );
            }
  ...
  const handleLeaveApplicationEmpEditUpdate = (info, newInfo) => {
    ...
    axios
      .put(`${apiUrl}/leave-application-emp/` + info["_id"], body, {
        headers: {
          authorization: localStorage.getItem("token") || "",
        },
      })
